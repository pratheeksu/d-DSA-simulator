...
def compute_overhead_new(time):
    ...
    if protocol == "XChant" or protocol == "HotPotato" or protocol == "q_learning":
        return 1
...
def compute_total_band_usage(t):
    ...
    with open(path_to_metrics + delivered_file, "r") as f:
        ...
        for line in lines:
            line_arr = line.strip().split()
            if int(line_arr[4]) <= t:
                bands_used[0] += int(line_arr[8])
                bands_used[1] += int(line_arr[9])
                bands_used[2] += int(line_arr[10])
                bands_used[3] += int(line_arr[11])
