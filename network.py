...
class Network(object):
            ...
            if p_user.is_active():
                for node in self.nodes:
                    ...
                    #                               float(p_user.x), float(p_user.y))
                        ...
                        dist1 = euclideanDistance(float(node.coord[0]), float(node.coord[1]),
                                                  float(p_user.x), float(p_user.y))
    ...
    def fill_network(self, num_nodes):  # quickly fill network and randomly place nodes for testing purposes
        ...
        for i in range(num_nodes):  # create and add nodes to network
            ...
            # node.load_pkl() # for Lexington
            node.add_coords(x_co_ordinates[int(node.ID)], y_co_ordinates[int(node.ID)])
            self.add_node(node)
        ...
        for line in lines:
            ...
            if int(line_arr[5]) == time:
                ...
                for j in range(num_packets):
                    ...
                    self.nodes[src].handle_buffer_overflow(max_packets_in_buffer)
            ...
            if (int(msg_line_arr[5]) == t):  # if a new message needs to be generated at this time
                ...
                for packet_id in range(num_packets):
                    ...
                    if len(message.path) > 0:
                        ...
                        self.message_num += 1
                        self.nodes[curr].handle_buffer_overflow(max_packets_in_buffer)
    ...
    def q_learning_get_next_node_and_band(self, node, msg_des):
        if np.random.rand() <= max(node.epsilon[msg_des], node.epsilon_min):
            if node.action_space == []:
                return -1
            action_index = random.randrange(len(node.action_space))

        else:
            action_index = np.argmax(node.q_table[msg_des, :])
        node.epsilon[msg_des] = node.epsilon[msg_des] * node.epsilon_decay
        return action_index
            ...
            for s in S:
                ...
                for neighbouring_node in nodes_in_range:
                    if (int(neighbouring_node.ID), s) not in node.action_space:
                        ...
                        for i in range(NoOfDataCenters):
                            node.q_table = np.append(node.q_table, 0)
            node.q_table = np.reshape(node.q_table, (NoOfDataCenters, len(node.action_space)))
    ...
    def update_node_buffer(self, node, buf_copy):
        i = 0
        while(len(node.buf) != len(buf_copy)):
            if (int(node.buf[i].ID), int(node.buf[i].packet_id)) not in buf_copy:
                node.buf.remove(node.buf[i])
                i = 0
            else:
                i += 1

        # print("Node buf len :", len(node.buf), "buf copy len:", len(buf_copy))
        # for i in range(len(node.buf)):
        #     print("buf_copy", buf_copy[i], "node buf", (node.buf[i].ID, node.buf[i].packet_id))
    ...
    def network_GO(self, t, specBW, path_lines, spec_lines, msg_lines,
        ...
        if protocol == "XChant":
            ...
            for node in self.nodes:  # send all messages to their next hop
                ...
                # print("Time: ", t, "Buffer: ", node.buf)
                node_buf_copy = []
                node.buf_length.append(len(node.buf))
                for msg in node.buf:
                    node_buf_copy.append((int(msg.ID), int(msg.packet_id)))
                for msg in node.buf:
                    # print("Node buffer:", node.buf)
                    # print("time : ", t, "node : ", int(node.ID), "Buff length: ",len(node.buf), "Message last sent:", msg.last_sent)
                    # print("Executing msg:", msg, "at time t: ",t, "\n")
                    ...
                    if msg.last_sent < t:
                        ...
                        if len(msg.path) > 0 and '' not in msg.path:  # if the message still has a valid path
                            ...
                            if int(msg.ID) == debug_message:
                                ...
                                print("Queue length : ", len(self.nodes[next].buf))
                            ...
                            transfer_time, transfer_time_in_sec = node.compute_transfer_time(msg, s, specBW,
                                                                                             msg.curr, next, t)
                            # print("Transfer time: ", transfer_time_in_sec)
                            ...
                            if limited_time_to_transfer == True:
                                ...
                                if node.mes_fwd_time_limit + transfer_time_in_sec <= (
                                    ...
                                    msg_sent = node.send_message_xchant(self, msg, t, specBW, LINK_EXISTS, next, s,
                                                                        transfer_time_in_sec, node_buf_copy)
                                    ...
                                    # print("Message fwd time: ", node.mes_fwd_time_limit)
                                else:
                                    if int(msg.ID) == debug_message:
                                        print("Time insufficient----, Msg fwd time: ", node.mes_fwd_time_limit, "Transfer time:", transfer_time_in_sec)
                            else:
                                msg_sent = node.send_message_xchant(self, msg, t, specBW, LINK_EXISTS, next, s,
                                                                    transfer_time_in_sec, node_buf_copy)
                                ...
                                if msg_sent == True:
                                    self.packets_per_tau += 1
                                    did_node_transmit = True
                self.update_node_buffer(node, node_buf_copy)
            ...
            for node in self.nodes:
                ...
                for msg in node.buf:
                    ...
                    if msg.last_sent < t:
                        ...
                        msg_des_index = int(msg.des) % NoOfDataCenters
                        action = self.q_learning_get_next_node_and_band(node, msg_des_index)
                        if action == -1:
                            pass
                        else:
                            next, s = node.action_space[action]
                            if int(msg.ID) == debug_message:
                                print("Message ID: ", msg.ID, "Message size:", msg.size, "Next: ", next, "Band: ", s)
                            ...
                            if (next == node.ID):
                                ...
                                # check if there is enough time to send message packet
                                queue_delay = len(self.nodes[next].buf) * packet_size/maxBW[0]
                                ...
                                reward = -2*np.arcsinh(queue_delay)
                                ...
                                if limited_time_to_transfer == True:
                                    ...
                                    if node.mes_fwd_time_limit + transfer_time_in_sec <= 1*(
                                            num_sec_per_tau * num_transceivers):
                                        ...
                                        if msg_sent == True:
                                            ...
                                            #     intF(node.ID), next, s, t] + 1 # 1 is for channel availability
                                            ...
                                            reward = reward + 2*np.arcsinh(-transfer_time_in_sec) + 2*np.arcsinh(dist_to_des_from_curr - dist_to_des)


                                            did_node_transmit = True
                                        else:
                                            transceiver, chan_avail = node.check_for_available_channel(node, self.nodes[next], t, self, s, LINK_EXISTS, transfer_time_in_sec)
                                            if chan_avail < 0 :
                                                reward = reward - 10
                                    else:
                                        if int(msg.ID) == debug_message:
                                    ...
                                    if msg_sent == True:
                                        ...
                                        # reward = reward + 1*(np.tanh(normalized_dist_difference)) + np.tanh(1-normalized_transfer_time_in_sec)
                                        reward = reward + 3*np.arcsinh(-transfer_time_in_sec) + np.arcsinh(
                                            dist_to_des_from_curr - dist_to_des)
                                        if next == int(msg.des):
                                            reward = reward + 5
                                        else:
                                            reward = reward - 0.5
                            ...
                            if int(msg.ID) == debug_message:
                                print("Queue length at current node: ", len(node.buf), "Queue length at next node:", len(self.nodes[next].buf))
                                print("Reward: ", reward, "Queue delay: ", np.arcsinh(queue_delay), "Transfer time:", transfer_time_in_sec)
                            ...
                            node.q_table[msg_des_index, action] = (1 - node.alpha) * node.q_table[msg_des_index, action] + node.alpha * (
                                    reward + node.gamma * np.max(self.nodes[next].q_table[msg_des_index, :]))
