...
class Node(object):
    def __init__(self, name):
        ...
        self.epsilon = [1] * NoOfDataCenters
            ...
            # if (node2.can_receive[i][0] == np.inf or node2.can_receive[i][0] == int(node1.ID)):
                ...
                for j in range(num_channels):
                    ...
                    if available == True: # if channel is available then update the network
                        ...
                        return i, j
            # else:
            #     print("T:", node2.can_receive[i], "Node 1 ID: ", node1.ID)
    ...
    def compute_transfer_time(self, msg, s, specBW, i, j, t):
        ...
        # time_to_transfer = math.ceil(transmission_time)
    ...
    def send_message_xchant(self, net, message, ts, specBW, LINK_EXISTS, next, s, transfer_time_in_sec, node_buf_copy): # xchants only
        nodes = net.nodes
    ...
    def send_message_qlearning(self, net, message, ts, specBW, LINK_EXISTS, next, s, transfer_time_in_sec, node_buf_copy): # q_learning only
        nodes = net.nodes
